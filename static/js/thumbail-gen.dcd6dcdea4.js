...
;// CONCATENATED MODULE: ./src/tools/thumbnail-gen/style.css
// extracted by css-extract-rspack-plugin

;// CONCATENATED MODULE: ./src/tools/thumbnail-gen/index.js
...
const THUMB_AT_MS = 15000;
const RECORD_AT_MS = 15000;
const RECORD_FOR_MS = 10000;
const CAPTURE_FPS = 5;
...
function buildAnimatedWebP(webpFrameArrays, width, height, frameDurationMs) {
    ...
    const vp8x = riffChunk('VP8X', cat(new Uint8Array([
        0x02,
        0x00,
        0x00,
        0x00
    ...
    // ── ANIM — 6-byte payload ───────────────────────────────
    const anim = riffChunk('ANIM', cat(new Uint8Array([
        0xFF,
        0xFF,
        0xFF,
        0xFF
    ...
    const anmfChunks = webpFrameArrays.map((bytes)=>{
        ...
        return riffChunk('ANMF', cat(u24(0), u24(0), u24(width - 1), u24(height - 1), u24(frameDurationMs), new Uint8Array([
            0x00
        ]), bitstream));
    });
    // ── RIFF container ──────────────────────────────────────
 ...
 */ const INJECT_SCRIPT = (/* unused pure expression or super */ null && (`
<script type="module">
...
// ── Show result in UI ─────────────────────────────────────
function showResult(project, param) {
    let { thumbnail, gifURL } = param;
...
async function runAll() {
    ...
    const queue = selected === 'all' ? PROJECTS : PROJECTS.filter((p)=>p.id === selected);
