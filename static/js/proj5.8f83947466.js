...
375(module, __unused_rspack___webpack_exports__, __webpack_require__) {
...
const DISPATCH_COUNT = (/* unused pure expression or super */ null && ([
    Math.ceil(NUM_AGENTS / WORKGROUP_SIZE),
    1,
    1
]));
const GRID_SIZE = 2;
...
// ==============================================================================
function makeVant() {
    let rules = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : [], { color = [
        1,
        0.15,
        0.05
...
// ==============================================================================
function defineChunk(param) {
    let { center = [
        0.5,
        0.5
    ], vants = [] } = param;
    const entries = Array.from({
        length: VANTS_PER_CHUNK
    }, (_, i)=>{
        const entry = vants[i] ?? {};
        return {
...
for (const chunk of chunks){
    for (const entry of chunk.vants){
        ...
        if (!rulesetOffset.has(def)) {
            rulesetOffset.set(def, rulesetList.length > 0 ? rulesetOffset.get(rulesetList[rulesetList.length - 1]) + rulesetList[rulesetList.length - 1].rules.length : 0);
...
const render_shader = https_cbcdn_githack_com_charlieroberts_gulls_raw_branch_main_gulls_js__rspack_import_0["default"].constants.vertex + `
