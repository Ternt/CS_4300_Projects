...
function extractGlowColor(imgEl, callback) {
  ...
  if (imgEl.complete && imgEl.naturalWidth > 0) sample();
    ...
    imgEl.addEventListener('load',  sample,          { once: true });
    imgEl.addEventListener('error', () => callback(null), { once: true });
  }
}

// ── Build one card ────────────────────────────────────────
function createCard({ name, url, thumbnail, gif, glowColor, description }) {
